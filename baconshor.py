...
def add_y_logical(grid,positions):
...
def number_of_y_logicals_from_distance(d):
    list_of_k  = []
...
def count_y_logicals_dict(listOfGrids, d):
    dictionary = {}
    possible_y_logicals = number_of_y_logicals_from_distance(d)
    for grid in listOfGrids:
        if check_config(grid):
            count = count_y_logicals(grid) 
...
def print_format(d,gridDictionary):
    ...
    for key in gridDictionary:
        print("# of y-logicals: ", key, " | Number of combinations: ", len(gridDictionary[key]))
