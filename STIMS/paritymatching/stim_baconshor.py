...
def bacon_shor_circuit(d,p):
    ...
    for col in range(d-1):
        for row in range(d):
            cirq.append("MXX", [int(d*row + col), int(d*row + col+1)]) 
        cirq.append("TICK")


    for i in range(d * d):
        # Apply Y error to every qubit with probability p
        cirq.append("Y_ERROR", i, p)
        
        # Apply background Z error to every qubit
        cirq.append("Z_ERROR", i, 1e-7)
        cirq.append("X_ERROR", i, 1e-7)
    ...
    for row in range(d-1):
        ...
        rec_targets = [stim.target_rec(-i) for i in range(1,d+1)]
        rec_targets.extend([stim.target_rec(-i) for i in range(offset+1, offset+d+1)])
        cirq.append("DETECTOR", rec_targets, arg = (col+0.5,d/2,0,1.0))
    ...
    for col in range(d-1):
        ...
        rec_targets = [stim.target_rec(-i) for i in range(1,d+1)]
        rec_targets.extend([stim.target_rec(-i) for i in range(offset+1, offset+d+1)])
        cirq.append("DETECTOR", rec_targets, arg = (col+0.5,d/2,0,2.0))
    ...
    # for col in range(d):
        # print(d*0+col)
    # cirq.append("OBSERVABLE_INCLUDE", [f"X{(d * row + 0)}" for row in range(d)], 0)

    # Track logical Z along the first column (vertical)
    # for row in range(d):
    #     cirq.append("OBSERVABLE_INCLUDE", [d * row + 0], "Z")
    cirq.append("OBSERVABLE_INCLUDE", [f"Z{(d*0+col)}" for col in range(d)], 0)
    return cirq

def bacon_shor_circuit_manual_errors(d,error_positions):
    cirq = stim.Circuit()
    ...
    for e in error_positions:
        cirq.append("Y_ERROR", e, 1.0)
    for i in range(d * d):
        
        # Apply background X and Z error to every qubit
        cirq.append("Z_ERROR", i, 1e-7)
        cirq.append("X_ERROR", i, 1e-7)
    cirq.append("TICK")
